...
const { createUsageLoggerSpy, sentinelLogger, observerCalls, reflectorCalls, dropperCalls } =
  vi.hoisted(() => ({
...
import { runConsolidationPipeline, type ConsolidationCtx } from "../src/om/consolidation.js";
...
describe("runConsolidationPipeline usageLog wiring", () => {
  it("stops cleanly when the session context becomes stale while creating the logger", async () => {
    ...
    await expect(
      runConsolidationPipeline(
        ...
        runtime,
        ctx,
        runtime.captureGeneration(SESSION_ID),
      ),
  ...
  it("forwards the same logger to every observer fallback attempt", async () => {
    ...
    (runObserver as any).mockImplementationOnce(async (args: { onAssistantMessage?: unknown }) => {
      observerCalls.push(args);
    ...
    expect(observerCalls.every((call) => call.onAssistantMessage === sentinelLogger)).toBe(true);
  });
  ...
  it("forwards the same logger to reflector and dropper stages", async () => {
    ...
    await runConsolidationPipeline(fakePi, runtime, ctx, runtime.captureGeneration(SESSION_ID));
